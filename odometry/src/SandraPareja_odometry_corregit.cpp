...
public:
  ...
  void run() {
    ...
    double delta_x  = 0; // JS: OK
    double delta_y  = 0;
    double delta_th = 0;
    double Delta_x  = 0; // JS: These are class members -- remove from here
    double Delta_y  = 0;
    double Delta_th = 0;
    C_D = Eigen::Matrix3d::Zero(3, 3); // JS: This is class member -- remove
    double x  = 0; // JS: These are class members -- remove from here
    double y  = 0;
    double th = 0;
    ...
    while(n.ok()) {
      if (N = A) {
        keyframe_callback(); // JS: with this call you will reset class members, but not your local variables defined above.
        N = 0;
      }
      usleep(100);
      current_time   = ros::Time::now();
      ...
      J_dvdt << 1, 0, 0, 0, 1, 0, 0, 0, 1; // JS: you can also use Eigen::Matrix3d::Identity();
      //J_dvdt(0,0) = 1;
      //J_dvdt(1,1) = 1;
      //J_dvdt(2,2) = 1;
